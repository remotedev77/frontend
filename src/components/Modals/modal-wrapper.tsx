  ...
  return (
    <LaberWrapper>
      {x.fileName ? (
        ...
        <>
          {x.description ? x.description : "загрузить файл"}
          <svg
...
export const ModalWrapper: FC<ModalWrapper> = observer((x) => {
  ...
  const { isOpen } = useOutSideClick(ref, true);
  useEffect(() => {
    !isOpen && x.onClick();
  }, [isOpen, x]);
