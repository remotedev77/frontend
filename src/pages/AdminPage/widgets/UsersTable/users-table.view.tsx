...
import { Cell, Dropdown, MainFilterControlls, Row, SubFilters, Table } from "../../components";
import { CsvModal, NewUser, UserInfoModal } from "../Modals";
import { AddButton, FilterButton, FilterSectionContainer, SearchBar } from "../../components";
  ...
  const Filters = observer(() => {
    return (
      <FilterSectionContainer>
        <MainFilterControlls>
          <SearchBar placeholder="Поиск по ФИО" onSearchChange={vm.onSearchChange} onSearch={() => vm.filter(data)} />
          <AddButton
            type="button"
            onClick={() => {
        ...
        <SubFilters>
          ...
          <Dropdown
            onOptionChange={vm.setFilterOrganization}
            filter={() => vm.filter(data)}
            placeholder="Организация"
            options={x.vm.companies.map((c) => {
              return { value: c.company_name, key: c.company_name };
            })}
          />
          <Dropdown
            onOptionChange={vm.setFilterAccess}
            filter={() => vm.filter(data)}
        ...
        {vm.getUsers().map((u, i) => {
          return (
            <Row
              ...
              onClick={() =>
                (x.vm.CurrentModal = (
                  <UserInfoModal
                    ...
                    user={u}
                    vm={vm}
                    company={x.vm.companies.find((c) => c?.id === Number(u?.organization))}
                  />
                ))
              }>
              <Cell>{u.id}</Cell>
              ...
              <Cell>{new Date(u.start_date ? u.start_date : "").toLocaleDateString()}</Cell>
              <Cell>{new Date(u.end_date ? u.end_date : "").toLocaleDateString()}</Cell>
              <Cell>
                {
                  u.organization
