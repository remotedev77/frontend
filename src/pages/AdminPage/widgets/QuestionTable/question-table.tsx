...
export const QuestionTable: FC<QuestionTableProps> = observer((x) => {
  ...
  const getKey = (pageIndex: number, previousPageData: DataFormat) => {
    ...
    if (pageIndex === 0) return `/questions/`;
  ...
  const Filters = observer(() => {
    return (
      <FilterSectionContainer>
        <MainFilterControlls>
          <SearchBar placeholder="Поиск" onSearchChange={vm.onSearchChange} onSearch={() => vm.filter(data)} />
          <AddButton
            type="button"
            onClick={() => {
    ...
    return (
      <>
        {vm.getQuestions().map((q, i) => (
          <Row onClick={() => (x.vm.CurrentModal = <QuestionInfo question={q} vm={vm} admin={x.vm} />)} key={i}>
