...
export const ManagersTable:FC<UsersTableProps> = observer(x=>{
    ...
    const Filters = observer(() =>{
        return<FilterSectionContainer>
            <MainFilterControlls>
                <SearchBar placeholder="Поиск по ФИО" onSearchChange={vm.onSearchChange} onSearch={()=>{}} />
    ...
    const MappedInfo = observer(() =>{
        return<> {vm.getManagers()
            .map((m,i)=> {
                return<Row key={i}
                           onClick={()=>x.vm.CurrentModal = <ManagerInfoModal vm={vm} admin={x.vm} manager={m}/>}
                >
