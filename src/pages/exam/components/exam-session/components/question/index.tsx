...
const Question = () => {
  const { selectedIndex, getSelectedQuestion, checkedQuestions } = useExamStore();
      ...
      <CardContent className="grid w-full gap-4">
        {getSelectedQuestion()?.answers.map((answer, index) => (
          <Answer
            ...
            {...answer}
            index={index}
            disabled={checkedQuestions.find(({ checkedIndex }) => selectedIndex === checkedIndex) ? true : false}
          />
        ))}
      </CardContent>
    </Card>
